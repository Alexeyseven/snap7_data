...
def valves():
    ...
    top.resizable(0,0)
    frame = Frame(top, width="950",height="500")
    frame.pack()
    swin = ScrolledWindow(frame, width=950, height=500)
    swin.pack()
    win = swin.window
    canvas1 = Canvas(win, width=950, height=16000) 
    canvas1.pack(side=LEFT, fill=BOTH, expand=True)
    Label(top, text = "                    V01        V02        V03        V04        V05        V07        V30        V31        V32        V33        V34        V35        V36        V37        V38        V39        V40        V41        V42        V43").place(x = 10, y=10)

    with sqlite3.connect("database.db") as db:
        
        cursor_valves = db.cursor()
        query_valves = """SELECT time, V01, V02, V03, V04, V05,
                V07, V30, V31, V32, V33, V34, V35, V36, V37, V38, V39,
                V40, V41, V42, V43
                FROM Valves_Keg
                WHERE (date = '%(day)s')
                """ %{'day' : day}
...
def mnemo():
    img = Image.open('Keg_mnemo.png')
    img.show()
    
root = Tk()
root.title('Paster Keg')
