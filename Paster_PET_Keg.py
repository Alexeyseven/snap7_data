...
def valves():
    ...
    top = Toplevel(root)
    top.title('Valves')
    top.geometry('870x500')
    top.resizable(0,0)
    frame = Frame(top, width="870",height="500")
    frame.pack()
    swin = ScrolledWindow(frame, width=870, height=500)
    swin.pack()
    win = swin.window
    canvas1 = Canvas(win, width=870, height=16000) 
    canvas1.pack(side=LEFT, fill=BOTH, expand=True)
    Label(top, text = "             V01-05   V01-07   V01-08   V01-09   V01-10   V01-11   V01-12  V01-51  V01-57   V02-01   V02-02 V02-04   V02-05   V02-08   V02-09    V60      V61     V80").place(x = 10, y=10)

    with sqlite3.connect("database.db") as db:
        
        cursor_valves = db.cursor()
        query_valves = """SELECT time, V01_05, V01_07, V01_08, V01_09,
                V01_10, V01_11, V01_12, V01_51, V01_57, V02_01, V02_02,
                V02_04, V02_05, V02_08, V02_09, V60, V61, V80
                FROM Valves_PET_Keg
                WHERE (date = '%(day)s')
                """ %{'day' : day}
...
def mnemo():
    img = Image.open('PET-Keg_mnemo.png')
    img.show()
    
root = Tk()
root.title('Paster PET Keg')
