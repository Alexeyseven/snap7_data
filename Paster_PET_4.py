...
def valves():
    ...
    Label(top, text = "                    V01        V02        V03        V04        V05        V06        V07        V08        V13        V16        V30        V31        V32        V33        V34        V35        V36        V38        V39        V40").place(x = 10, y=10)

    with sqlite3.connect("database.db") as db:
        
        cursor_valves = db.cursor()
        query_valves = """SELECT time, V01, V02, V03, V04, V05, V06,
                V07, V08, V13, V16, V30, V31, V32, V33, V34, V35, V36,
                V38, V39, V40
                FROM Valves_PET_4
                WHERE (date = '%(day)s')
                """ %{'day' : day}
...
def mnemo():
    img = Image.open('PET_4_mnemo.png')
    img.show()
    
root = Tk()
root.title('Paster PET-4')
